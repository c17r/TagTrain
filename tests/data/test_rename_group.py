...
def test_unknown_owner(database):
    with pytest.raises(data.Group.DoesNotExist):
        group = data.rename_group('non-existent', db.GROUP_NAME, 'doesnt-matter')


def test_unknown_group(database):
    with pytest.raises(data.Group.DoesNotExist):
        group = data.rename_group(db.OWNER_NAME, 'non-existent', 'doesnt-matter')
...
def test_good(database):
    ...
    with pytest.raises(data.Group.DoesNotExist):
        group = data.find_group(db.OWNER_NAME, NEW_NAME)

    data.rename_group(db.OWNER_NAME, db.GROUP_NAME, 'new-name')

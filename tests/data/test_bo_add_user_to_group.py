...
def test_new_group(database):
    ...
    group, created = data.by_owner.add_user_to_group(db.OWNER_NAME, 'new-group', 'new-member')
...
def test_new_owner(database):
    groups = list(data.by_owner.find_groups('new-guy'))

    assert len(groups) == 0

    group, created = data.by_owner.add_user_to_group('new-guy', 'new-group', 'new-member')
...
def test_new_member(database):
    ...
    group, created = data.by_owner.add_user_to_group(db.OWNER_NAME, db.GROUP_NAME, 'new-member')
...
def test_existing_member(database):
    ...
    group, created = data.by_owner.add_user_to_group(db.OWNER_NAME, db.GROUP_NAME, 'one')
